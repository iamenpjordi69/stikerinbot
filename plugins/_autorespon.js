...
handler.all = async function (m, { isBlocked }) {
    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'MilfBOT is Active' : banned ? 'You are banned' : 'MilfBOT is Active',
                '© stikerin',
    ...
    if ((m.mtype === 'groupInviteMessage' || m.text.startsWith('https://chat') || m.text.startsWith('Open this link')) && !m.isBaileys && !m.isGroup) {
        this.sendButton(m.chat, `┌「 Add Bots to your Group 」
├ Invite Bot via Invitation
...
`.trim(), '© MilfBOT', 'Bot Owner', ',owner', m)
    ...
    if (set.autoupdatestatus) {
        if (new Date() * 1 - set.status > 1000) {
            ...
            await this.setStatus(`Active Since ${uptime} | Mode: ${set.self ? 'Private' : set.group ? 'Group only' : 'Public'} | MilfBOT by Jordi`).catch(_ => _)
