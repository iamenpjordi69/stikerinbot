let handler = async (m, { conn, text }) => {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `Enter number, tag or reply to target message.`, m)
  // let exists = await conn.isOnWhatsApp(number)
  // if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
  if(isNaN(number)) return conn.reply(m.chat, `The number you entered is wrong!`, m)
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `*Can't date myself t_t`, m)
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`You just refused @${user.split('@')[0]} to be your partner!`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
