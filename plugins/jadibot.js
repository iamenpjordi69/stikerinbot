...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'https://i.ibb.co/W0RWQQf/logo-bot-host.jpg', 'Scan QR ini untuk jadi bot sementara\n\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk Perangkat Tertaut\n3. Tautkan Perangkat\n4. Scan QR ini \nQR Kadaluarsa dalam 20 detik', m)
      setTimeout(() => {
